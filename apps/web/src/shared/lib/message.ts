...
export const onSuccess = createEvent<Parameters<MessageInstance['success']>[0]>();

export const $messageApi = createStore<MessageInstance | null>(null);

export const errorFx = attach({
  source: $messageApi,
  effect: (messageApi, payload: Parameters<MessageInstance['error']>[0]) => messageApi?.error(payload)
})
