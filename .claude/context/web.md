...
- **UI**: (component library not explicitly shown — uses custom components)
...
Each module wraps Axios calls for a domain:
- `auth.ts` — Google OAuth redirect helpers
- `user.ts` — GET /me, GET /:id, PUT, PATCH /avatar
- `tree.ts` — CRUD for family trees
- `tree-member.ts` — CRUD for members (child/spouse/parents/update/delete)
- `tree-member-connection.ts` — GET connections
- `shared-tree.ts` — shared trees, shared users, update RBAC
- `notification.ts` — GET notifications, mark read
- `fcm-token.ts` — POST/DELETE FCM tokens
- `file.ts` — POST file upload
...
- Uses `family-chart` library (custom CSS in `shared/styles/family-chart-custom.css`)
...
- `shared/lib/create-form.ts` — generic form factory (likely wraps effector-forms or similar)
- `shared/lib/disclosure.ts` — open/close state for modals/drawers
- Lazy loading via `shared/lib/lazy-page.ts` + `with-suspense` HOC
- `appStarted` event (from `shared/config/system.ts`) fires on app boot to initialize router history
